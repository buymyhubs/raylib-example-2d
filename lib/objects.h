...
class entity : public object {
    ...
    public:
        ...
        void set_entity_x(const float x) {
            ...
            const Rectangle check_rectangle = {x, get_y(), get_collision_rectangle().width, get_collision_rectangle().height};

            if (auto [can_move, closest_location] = check_all_collision(check_rectangle, solids, direction, true); can_move) {
                set_x(x);
            } else {
